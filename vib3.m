...
%allows user input for frequency
...
if noteoraudio == 1
    ...
    if tremorvib == 1
        ...
        soundsc(notevibe, rate); %plays notevib 
    else
        depth = input('Enter value for depth (value greater than 0): '); %user input for depth (change amplitude of phase shift)
        vibra = sin(2*pi*freq*time+depth*sin(2*pi*fv*time)); %adds a phase shift to note
    ...
    audio'; %changes a vertical vector to horizontal vector
